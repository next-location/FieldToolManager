...
2.  **破壊的変更の事前承認:**
    API 仕様、DB カラム削除、重要ロジック変更は、必ず事前に承認を得ること。
3.  **トークン節約:**
...
- **行動フロー:**
  ...
  3. **【重要】DB 変更があった場合、「`MIGRATIONS.md` は更新しましたか？」と担当に確認する。**
  4. 最後に「デプロイしますか？」とユーザーに確認する。
...
- **デプロイ手順:**
  - ユーザーから「デプロイして」と指示があった場合、あるいは PM の指示で、以下の順序で実行する。
  1. `npm run build` (ビルドチェック)
  2. `git add .`
  3. `git commit -m "書記が作成したメッセージ"`
  4. `git push origin main` (これで Vercel が自動デプロイ)

### 💳 5. バックエンド・決済担当

- **役割:** DB(Supabase)、Stripe、API 開発。
- **⚠️ 完了条件 (Definition of Done):**
  - データベースやテーブルを変更した際は、**必ずその実行した SQL を `docs/MIGRATIONS.md` に追記すること。**
  - これを行わずに「完了しました」と報告することは禁止。
  - TypeScript の型定義（Types）も同期させること。

### 💻 6. フロントエンド実装担当

- **役割:** Next.js(React)の実装。UI 維持を最優先。
