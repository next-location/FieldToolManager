...
export async function notifyCheckoutReminder(params: NotifyCheckinReminderParams) {
  ...
  const notifications = recipients.map((recipient) => ({
    organization_id: params.organizationId,
    type: '出退勤アラート',
    title: `退勤打刻忘れアラート（${params.missingUsers.length}名）`,
    message: `${params.targetDate}の退勤打刻がまだ行われていません：${summary}`,
    severity: 'warning' as const,
    target_user_id: recipient.id,
    sent_via: ['in_app'],
    sent_at: new Date().toISOString(),
  ...
  if (insertError) {
    ...
    console.log(`[通知] ${recipients.length}人の管理者に退勤アラート通知を送信しました`)
  }
}

/**
 * 個別の退勤打刻忘れ通知をスタッフ本人に送信
 */
export async function notifyIndividualCheckoutReminder(params: {
  organizationId: string
  userId: string
  userName: string
  targetDate: string
