...
### 🎓 リリース前の設定手順（中学生でもわかる完全ガイド）

この設定は**リリース日の前日**に行います。所要時間は約15分です。

---

#### 📦 STEP 1: Supabaseを有料プランに変更する（5分）

**目的**: データベースの自動バックアップ機能を使えるようにする
...
2. **プロジェクトを選択**
   - 画面左上にプロジェクト名が表示されているのでクリック
   - 「FieldToolManager」のような名前のプロジェクトを選ぶ

3. **有料プランに変更**
   - 画面左下の「Settings」（⚙️歯車マーク）をクリック
   - 「Billing & Usage」をクリック
   - 「Upgrade to Pro」ボタンを探してクリック
   - クレジットカード情報を入力（月額$25が請求されます）
   - 「Subscribe」ボタンをクリック

4. **確認**
   - プラン名が「Pro」になっていることを確認
   - これで**毎日自動的にバックアップが取られる**ようになります！

**💰 費用**: 月額$25（約3,500円）

---

#### 🗄️ STEP 2: バックアップ保存用のフォルダを作る（3分）

**目的**: 自動バックアップしたファイルを保存する場所を作る

1. **Supabase Dashboardで「Storage」を開く**
   - 画面左側のメニューから「Storage」（📦箱マーク）をクリック
...
2. **新しいバケット（フォルダ）を作る**
   ...
   - 以下を入力：
     ...
     ```
   - 「Public bucket」のスイッチが**OFF**（灰色）になっていることを確認
     - ⚠️ ONにしないでください！誰でもダウンロードできてしまいます
   - 「Save」ボタンをクリック

3. **確認**
   - 左側のリストに「organization-backups」が表示されていればOK

---

#### 🔐 STEP 3: Vercelの環境変数を確認する（5分）

**目的**: 自動バックアップを動かすための「秘密の鍵」を設定する
...
2. **プロジェクトを開く**
   - 「FieldToolManager」プロジェクトをクリック

3. **環境変数の設定画面を開く**
   - 画面上部の「Settings」タブをクリック
   - 左メニューから「Environment Variables」をクリック

4. **CRON_SECRETが設定されているか確認**
   - リストから「CRON_SECRET」を探す
   - **見つかった場合**: 何もしなくてOK（次のSTEPへ）
   - **見つからない場合**: 以下の手順で追加

5. **CRON_SECRETを追加する（見つからなかった場合のみ）**

   **方法A: 自分のパソコンで生成する場合**
   - ターミナル（Mac）またはコマンドプロンプト（Windows）を開く
   - 以下のコマンドを入力してEnterキーを押す：
     ```bash
     openssl rand -hex 32
     ```
   - 表示された長い文字列（例：a1b2c3d4e5f6...）をコピー

   **方法B: ウェブサイトで生成する場合**
   - https://generate-secret.now.sh/32 を開く
   - 表示された文字列をコピー

   **Vercelに設定**
   - Vercelの画面に戻る
   - 「Add New」ボタン → 「Environment Variable」をクリック
   - 以下を入力：
     ```
     Key: CRON_SECRET
     Value: （先ほどコピーした長い文字列を貼り付け）
     ```
   - 「Production」にチェックが入っていることを確認
   - 「Save」ボタンをクリック

6. **再デプロイする（CRON_SECRETを追加した場合のみ）**
   - 画面上部の「Deployments」タブをクリック
   - 一番上の項目の右側にある「︙」（3つの点）をクリック
   - 「Redeploy」をクリック
   - 「Redeploy」ボタンをもう一度クリックして確認
   - 5分ほど待つとデプロイ完了
...
**Phase 1: データエクスポート機能のテスト**

1. **本番サイトにアクセス**
   - https://zairoku.com にログイン
   - 管理者アカウントでログインしてください

2. **データエクスポートページを開く**
   - 左側のメニューから「データエクスポート」をクリック
   - URL: https://zairoku.com/settings/data-export

3. **テストダウンロード**
   - 「道具マスタをエクスポート」ボタンをクリック
   - CSVファイルがダウンロードされればOK！
   - Excelで開いて文字化けしていないことを確認

**Phase 2: 自動バックアップ機能の確認**

- 自動バックアップは**毎日午前3時**に自動実行されます
- 翌朝、Supabase Dashboard → Storage → organization-backups を確認
- バックアップファイル（`.json.gz`）が保存されていればOK！

---

#### 🎁 おまけ: PITR（超高機能バックアップ）を追加する（オプション）

**目的**: 「1分前の状態に戻したい！」ができるようになる

**こんな人におすすめ**:
- 月額+$100（約14,000円）払える
- データが超重要で、1日分のデータも失いたくない

**設定方法**:

1. Supabase Dashboard → Settings → Addons
2. 「Point in Time Recovery」を探す
3. 「Enable PITR」ボタンをクリック
4. 「Enable」をクリックして確認

**💰 追加費用**: 月額$100（約14,000円）

**メリット**:
- 過去7日間の**任意の時点**に復元できる（秒単位）
- 例：「昨日の午後2時37分の状態に戻す」ができる
...
□ Supabaseを有料プラン（Pro）に変更した
□ Supabase Storageで「organization-backups」バケットを作った
□ Vercelで「CRON_SECRET」環境変数を確認した
□ 本番サイトでデータエクスポートをテストした
□ （オプション）PITRを有効にした

リリース翌朝にやること:

□ Supabase Storage → organization-backups にバックアップファイルがあるか確認
```
...
A: Pro Planは月額$25固定です。データ量が増えても基本的に変わりません。追加費用が発生するのは以下の場合のみ：
- データベースサイズが8GBを超えた場合（超過分に追加料金）
- 月間50GBを超えるデータ転送が発生した場合

**Q: CRON_SECRETって何ですか？**

A: 「合言葉」のようなものです。自動バックアップを実行する際に「本当にシステム管理者からの命令か？」を確認するための秘密の文字列です。これがないと、他の人が勝手にバックアップを実行できてしまいます。

**Q: バックアップファイルはどこに保存されますか？**

A: Supabaseの「Storage」→「organization-backups」フォルダに保存されます。ファイル名は以下の形式：
```
backups/組織ID/2025-01-22T03-00-00-000Z_full_backup.json.gz
```

**Q: 自動バックアップはいつ実行されますか？**

A: 毎日午前3時（日本時間）に自動実行されます。寝ている間にバックアップが完了します。
