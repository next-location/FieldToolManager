...
| `leader` | リーダー | チーム管理と現場指揮を行う責任者 | 現場監督、チームリーダー |
| `manager` | マネージャー | 管理業務全般と分析を行う管理者 | 部門長、中間管理職 |
...
#### staff（現場スタッフ）
- **主要業務**: QRコードスキャン、道具の持出・返却、作業報告書作成
- **アクセス範囲**: 自分の操作履歴、基本的な道具情報、自分の勤怠記録
- **制限事項**: 管理機能へのアクセス不可、他人のデータ閲覧不可

#### leader（リーダー）
- **主要業務**: チーム管理、リーダー用QR発行、現場マスタ確認
- **アクセス範囲**: 自チームの勤怠管理、現場マスタ閲覧、チームの作業報告書
- **制限事項**: 道具登録・編集不可、システム設定不可、レポート分析不可

#### manager（マネージャー）
- **主要業務**: 道具・重機管理、全体勤怠管理、レポート分析、限定的なスタッフ管理
- **アクセス範囲**: 全員の勤怠データ、道具・重機の登録編集、各種分析レポート
- **制限事項**: システム設定不可、削除権限なし、adminユーザーの編集不可

#### admin（システム管理者）
- **主要業務**: 全機能の管理、マスタ設定、監査ログ確認
- **アクセス範囲**: 組織内の全データ・全機能、システム設定、監査ログ
- **制限事項**: 他組織のデータアクセス不可
...
| 機能ID | 機能名 | staff | leader | manager | admin |
|--------|--------|-------|--------|---------|-------|
| F001 | ログイン/ログアウト | F | F | F | F |
| F002 | パスワード変更 | L | L | L | L |
| F003 | プロフィール編集 | L | L | L | L |
| F004 | ダッシュボード表示 | F | F | F | F |
| F005 | QRコードスキャン | F | F | F | F |
| F006 | 道具持出登録 | F | F | F | F |
| F007 | 道具返却登録 | F | F | F | F |
| F008 | 道具検索 | R | R | F | F |
| F009 | 道具詳細表示 | R | R | F | F |
...
- 自チーム: 自分のチームのみアクセス可能
- 全員: 組織内全員にアクセス可能
- leader/staff: leader/staff役割のユーザーのみ操作可能

#### 基本機能

| 機能名 | staff | leader | manager | admin | 備考 |
|--------|-------|--------|---------|-------|------|
| ダッシュボード | ✅ | ✅ | ✅ | ✅ | |
| QRスキャン | ✅ | ✅ | ✅ | ✅ | |
| プロフィール | ✅ | ✅ | ✅ | ✅ | |
...
|--------|-------|--------|---------|-------|------|
| スタッフ一覧（閲覧） | ✅ | ✅ | ✅ | ✅ | |
| スタッフ追加 | ❌ | ❌ | leader/staff | ✅ | managerはleader/staffのみ |
| スタッフ編集 | ❌ | ❌ | leader/staff | ✅ | managerはleader/staffのみ |
| スタッフ削除 | ❌ | ❌ | leader/staff | ✅ | managerはleader/staffのみ |
| 権限変更 | ❌ | ❌ | ❌ | ✅ | adminのみ |
| パスワードリセット | ❌ | ❌ | ❌ | ✅ | adminのみ |
| 変更履歴閲覧 | ❌ | ❌ | ❌ | ✅ | adminのみ |
...
|--------|-------|--------|---------|-------|------|
| 取引先マスタ | ❌ | ❌ | ❌ | ✅ | adminのみ |
| **仕入先マスタ（閲覧）** | ❌ | ✅ | ✅ | ✅ | **Leader以上、DXパック必須** |
| **仕入先マスタ（編集）** | ❌ | ❌ | ❌ | ✅ | **adminのみ、DXパック必須** |
| 現場マスタ閲覧 | ❌ | ✅ | ✅ | ✅ | |
| 現場登録・編集 | ❌ | ❌ | ✅ | ✅ | |
| 現場削除 | ❌ | ❌ | ❌ | ✅ | adminのみ |
| 倉庫位置管理 | ❌ | ❌ | ❌ | ✅ | adminのみ |
| **マスタ管理（カテゴリ・道具・消耗品）閲覧** | ❌ | ❌ | ✅ | ✅ | **Manager/Admin、現場資産パック必須** |
| **マスタ管理（カテゴリ・道具・消耗品）編集** | ❌ | ❌ | ✅ | ✅ | **Manager/Admin、現場資産パック必須** |
| **マスタ管理（カテゴリ・道具・消耗品）削除** | ❌ | ❌ | ❌ | ✅ | **adminのみ、現場資産パック必須** |
| 月次勤怠レポート | ❌ | ❌ | ✅ | ✅ | |
| 取引先統計・分析 | ❌ | ❌ | ❌ | ✅ | adminのみ |
| コスト分析 | ❌ | ❌ | ✅ | ✅ | |
| 使用頻度分析 | ❌ | ❌ | ✅ | ✅ | |
...
| **発注書閲覧** | ❌ | ✅ | ✅ | ✅ | **Leader以上** |
| **発注書作成** | ❌ | ✅ | ✅ | ✅ | **Leader以上** |
| **発注書承認（10万円未満）** | ❌ | ✅ | ✅ | ✅ | **Leader以上** |
| **発注書承認（10万円以上100万円未満）** | ❌ | ❌ | ❌ | ✅ | **adminのみ** |
| **発注書承認（100万円以上）** | ❌ | ❌ | ❌ | ✅ | **adminのみ** |
| **発注書差戻し** | ❌ | ✅ | ✅ | ✅ | **Leader以上** |
| **発注書PDF出力** | ❌ | ✅ | ✅ | ✅ | **Leader以上** |
| **発注書CSV出力** | ❌ | ✅ | ✅ | ✅ | **Leader以上** |
| **発注分析レポート** | ❌ | ✅ | ✅ | ✅ | **Leader以上** |
...
### 4.3 管理機能権限（super_admin専用）

| 機能ID | 機能名 | staff | leader | manager | admin | super_admin |
|--------|--------|-------|--------|-------|-------------|
| F035 | 組織設定 | × | × | F | F |
| F036 | セキュリティ設定 | × | × | F | F |
| F037 | 監査ログ閲覧 | × | × | F | F |
