...
| メニュー項目 | URL | 状態 | 説明 |
|------------|-----|------|------|
| ダッシュボード | `/admin/dashboard` | ✅ 実装済み | 統計情報の概要 |
| 契約管理 | `/admin/contracts` | ✅ 実装済み | 全組織の契約一覧・管理 |
| 組織管理 | `/admin/organizations` | 🚧 未実装 | 組織の追加・編集・削除 |
| パッケージ設定 | `/admin/packages` | 🚧 未実装 | プラン・機能パックの設定 |
| 操作ログ | `/admin/logs` | 🚧 未実装 | スーパーアドミンの操作履歴 |
| 売上分析 | `/admin/analytics` | 🚧 未実装 | 売上・契約状況の分析 |
| 設定 | `/admin/settings` | 🚧 未実装 | システム設定 |
