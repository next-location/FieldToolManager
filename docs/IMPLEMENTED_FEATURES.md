...
4. ✅ **保証期限切れ通知（メール送信）**
   - toolsテーブルに warranty_expiration_date カラム追加
   - 保証期限切れメール送信機能（lib/email.ts）
   - HTMLメールテンプレート（期限切れ/間もなく期限切れ対応）
   - Cron Job API（/api/cron/check-warranty-expiration）
   - 30日前からアラート送信
   - 道具登録・編集フォームに保証期限フィールド追加
   - 通知履歴のDB保存
...
7. ✅ **複数道具の一括移動**
   - 移動先を先に選択してから複数道具を連続スキャン/選択
   - 倉庫→現場、現場→倉庫、現場→現場の3パターン対応
   - 選択した道具をリスト表示
   - 一括登録機能（進捗表示付き）
   - `/movements/bulk` ページに実装
   - BulkMovementForm.tsx コンポーネント
