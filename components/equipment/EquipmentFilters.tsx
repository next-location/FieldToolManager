...
import EquipmentFiltersModal from './EquipmentFiltersModal'
...
export default function EquipmentFilters({ onFilterChange, categories }: EquipmentFiltersProps) {
  ...
  const filterCount = [filters.categoryId, filters.ownershipType, filters.status].filter(Boolean).length

  const handleModalFilterChange = (key: keyof FilterState, value: string) => {
