...
export default function GenerateEstimateButton({
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/contracts/${contractId}/generate-estimate`, {
        method: 'POST',
        credentials: 'include',
      });
      ...
      if (!response.ok) {
        setResult({ success: false, error: data.error || '見積もりの生成に失敗しました' });
        return;
      }
      ...
      console.error('[GenerateEstimateButton] Error:', error);
      ...
      {showModal && !result && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-bold text-gray-900 mb-4">見積もりの生成</h3>
            <p className="text-gray-700 mb-2">契約番号: {contractNumber}</p>
            <p className="text-gray-700 mb-4">
              見積もりを生成します。
              <br />
              生成後、内容を確認してから顧客に送信できます。
            </p>
            ...
            {result.success ? (
              <>
                <div className="flex items-center mb-4">
                  ...
                  <h3 className="text-lg font-bold text-gray-900">見積もりを生成しました</h3>
                </div>
                <div className="bg-gray-50 rounded-lg p-4 mb-4">
                  <p className="text-sm text-gray-600 mb-2">見積書番号</p>
                  <p className="font-mono text-gray-900 mb-4">{result.estimateNumber}</p>
                  <p className="text-sm text-gray-600 mb-2">見積金額</p>
                  <p className="text-2xl font-bold text-gray-900">
                    ¥{result.amount?.toLocaleString()}
                  </p>
                </div>
                <p className="text-sm text-gray-600 mb-4">
                  内容を確認後、「見積もりを送信」ボタンで顧客に送信してください。
                </p>
