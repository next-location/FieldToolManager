...
export async function GET(
  ...
  console.log('[Purchase Order PDF API] Starting PDF generation for order:', id)
  ...
  try {
    ...
    if (error || !order) {
      console.log('[Purchase Order PDF API] Order not found:', error)
      return NextResponse.json({ error: '発注書が見つかりません' }, { status: 404 })
    }

    // 承認チェック: 下書き・提出済み状態の発注書は承認済みのみPDF出力可能
    if ((order.status === 'draft' || order.status === 'submitted')) {
      console.log('[Purchase Order PDF API] Order not approved yet')
      return NextResponse.json({
        error: 'この発注書は未承認のため、PDF出力できません。上司の承認が必要です。'
      }, { status: 403 })
    }

    console.log('[Purchase Order PDF API] Order found, generating PDF...')
    ...
    try {
      ...
      console.error('[Purchase Order PDF API] Font registration error:', fontError)
    }
    ...
    doc.text('発　注　書', 105, yPos, { align: 'center' })
    ...
    if (order.client?.postal_code) {
      doc.setFontSize(8)
      doc.text(`〒${order.client.postal_code}`, leftX, yPos)
      yPos += 4
    }
    if (order.client?.address) {
      doc.setFontSize(8)
      doc.text(order.client.address, leftX, yPos)
      yPos += 4
    }
    ...
    // 説明文（金額ボックスの少し上、小さく）
    doc.setFontSize(8)
    doc.text('下記のとおり発注いたします。', leftX, yPos)
    yPos += 5
    ...
    doc.setFontSize(8)
    doc.text('ご発注金額（税込）', leftX + 2, yPos + 5)
    doc.setFontSize(16)
    doc.text(`¥${Math.floor(order.total_amount).toLocaleString()}`, leftX + 2, yPos + 14)
    ...
    // 発注番号・登録番号・日付（さらに小さく、右端寄せ）
    doc.setFontSize(6)
    doc.text(`発注番号: ${order.order_number}`, rightEdge, rightY, { align: 'right' })
    rightY += 3
    ...
    doc.text(`発注日: ${new Date(order.order_date).toLocaleDateString('ja-JP')}`, rightEdge, rightY, { align: 'right' })
    rightY += 3
    if (order.delivery_date) {
      doc.text(`納期: ${new Date(order.delivery_date).toLocaleDateString('ja-JP')}`, rightEdge, rightY, { align: 'right' })
      rightY += 3
    }
    ...
    try {
      ...
      console.error('[Purchase Order PDF API] Error adding company seal:', err)
    }

    // 納品場所・支払条件セクション（上に移動、小さく）
    yPos = 95

    if (order.delivery_location) {
      doc.setFontSize(8)
      doc.text('納品場所', 15, yPos)
      yPos += 5
      doc.setFontSize(9)
      doc.text(order.delivery_location, 15, yPos)
      yPos += 4
    }

    if (order.payment_terms) {
      doc.setFontSize(8)
      doc.text('支払条件', 15, yPos)
      yPos += 5
      doc.setFontSize(9)
      doc.text(order.payment_terms, 15, yPos)
      yPos += 4
    }
    ...
    if (order.project?.project_name) {
      ...
      yPos += 5
      doc.setFontSize(9)
      doc.text(order.project.project_name, 15, yPos)
      yPos += 4
    }
    ...
    const items = order.purchase_order_items?.sort((a: any, b: any) => a.display_order - b.display_order) || []
    ...
    doc.text(`¥${order.subtotal.toLocaleString()}`, summaryX + summaryWidth, yPos, { align: 'right' })
    yPos += 5

    doc.text('消費税:', summaryX, yPos)
    doc.text(`¥${Math.floor(order.tax_amount).toLocaleString()}`, summaryX + summaryWidth, yPos, { align: 'right' })
    yPos += 5
    ...
    doc.text(`¥${Math.floor(order.total_amount).toLocaleString()}`, summaryX + summaryWidth, yPos, { align: 'right' })
    ...
    if (order.notes) {
      ...
      // テキストを分割
      doc.setFontSize(8)
      const noteLines = doc.splitTextToSize(order.notes, notesBoxWidth - (notesPadding * 2))
    ...
    const fileName = `発注書_${order.order_number}_${dateStr}.pdf`
    ...
    // PDFを返す
    console.log('[Purchase Order PDF API] PDF generated successfully')
    ...
    console.error('[Purchase Order PDF API] Error generating PDF:', error)
    return NextResponse.json(
      {
        error: 'PDF生成に失敗しました',
