...
export async function GET(
  ...
  try {
    ...
    // 前文
    doc.setFontSize(10);
    const lineHeight = 10 * 1.6; // フォントサイズ * 1.6
    ...
    preambleLines.forEach((line: string, i: number) => {
      doc.text(line, 20, yPos + i * lineHeight / 2);
    });
    yPos += preambleLines.length * lineHeight / 2 + 5;
    ...
    articles.forEach((article, articleIndex) => {
      // 最後の条文（第10条）以外で、タイトルが入るかチェック
      const isLastArticle = articleIndex === articles.length - 1;
      if (!isLastArticle && yPos > 270) {
        doc.addPage();
        yPos = 20;
      }
      ...
      doc.setFontSize(9);
      const itemLineHeight = 9 * 1.6; // フォントサイズ * 1.6
      article.items.forEach((item, index) => {
        // (1)のような括弧付き数字で始まる場合は、リスト番号を付けない
        const itemText = item.trim().match(/^（\d+）|^\(\d+\)/)
          ? item
          : `${index + 1}. ${item}`;
        const lines = doc.splitTextToSize(itemText, 165);

        // 最後の条文以外で、項目が途中で改ページされないようにチェック
        const itemHeight = lines.length * itemLineHeight / 2;
        if (!isLastArticle && yPos + itemHeight > 270) {
          doc.addPage();
          yPos = 20;
        }

        lines.forEach((line: string, i: number) => {
          doc.text(line, 25, yPos + i * itemLineHeight / 2);
        });
        yPos += itemHeight + 2; // 項目間の間隔を統一
      });
      yPos += 4; // 条文間の間隔を統一
    });
    ...
    doc.text('令和　　年　　月　　日', 140, yPos);
    yPos += 20;

    // 甲（ご利用者）
    doc.setFontSize(11);
    doc.text('（甲）ご利用者', 20, yPos);
    yPos += 7;
    doc.setFontSize(10);
    doc.text('住所：', 20, yPos);
    yPos += 6;
    doc.text('氏名：', 20, yPos);
    doc.text('印', 180, yPos - 2);
    yPos += 15;
