...
export async function POST(
  ...
  try {
    ...
    // document_typeがinvoiceであることを確認
    if (invoice.document_type !== 'invoice') {
      return NextResponse.json({ error: 'この文書は請求書ではありません' }, { status: 400 });
    }

    // statusがsentまたはpaidであることを確認（overdueも許可）
    if (!['sent', 'paid', 'overdue'].includes(invoice.status)) {
      return NextResponse.json({
        error: `請求書の再送信はsent/paid/overdueステータスでのみ可能です。現在のステータス: ${invoice.status}`
      }, { status: 400 });
    }
    ...
    try {
      ...
      console.error('[Invoice Resend] Email error:', emailError);
      return NextResponse.json(
    ...
    console.error('[Invoice Resend API] Error:', error);
    return NextResponse.json(
      { error: '請求書再送信に失敗しました', details: error.message },
      { status: 500 }
    );
  }
}
