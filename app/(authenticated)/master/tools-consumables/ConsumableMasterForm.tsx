...
import { createOrUpdateConsumableMaster } from './actions'
...
export function ConsumableMasterForm({
  categories,
  manufacturers,
  organizationId,
  editingMaster,
  ...
  const [formData, setFormData] = useState({
    ...
    minimum_stock: editingMaster?.minimum_stock?.toString() || '10',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createOrUpdateConsumableMaster({
        id: editingMaster?.id,
        name: formData.name,
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            name="manufacturer"
            id="manufacturer"
            list="manufacturers-list-consumable"
            value={formData.manufacturer}
            onChange={handleChange}
            className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500"
            placeholder="メーカー名を入力または選択..."
          />
          <datalist id="manufacturers-list-consumable">
        ...
        {/* 画像 */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">消耗品の画像</label>
          <ImageUpload
