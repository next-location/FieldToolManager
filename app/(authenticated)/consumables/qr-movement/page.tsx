...
import { ConsumableQRMovementForm } from './ConsumableQRMovementForm'

interface QRMovementPageProps {
  searchParams: Promise<{ item?: string }>
}

export default async function ConsumableQRMovementPage({ searchParams }: QRMovementPageProps) {
  const params = await searchParams
  const { userId, organizationId, userRole, supabase } = await requireAuth()

  // itemパラメータがない場合は一括移動ページへリダイレクト
  if (!params.item) {
    redirect('/consumables/bulk-movement')
  }

  const consumableId = params.item
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
      <div className="px-4 pt-3 sm:px-0 sm:py-6">
        <div className="mb-6">
          <h1 className="text-lg sm:text-2xl font-bold text-gray-900">消耗品移動（QRスキャン）</h1>
